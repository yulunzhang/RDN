...
function addDenseLayer(model, nDenseChannels, growthRate)
    model:add(nn.SpatialConvolution(nDenseChannels, growthRate, 3, 3, 1, 1, 1, 1))
...
function denseBlock(model, growthRate, scaleRes, ipMulc, nDenseConv)
    local nDenseConv = nDenseConv or 12
    local nDenseChannels = growthRate or 64
    local scaleRes = (scaleRes and scaleRes ~= 1) and scaleRes or false
	local ipMulc = ipMulc or false
	if not scaleRes then
		assert(not ipMulc, 'Please specify -scaleRes option')
	end
    -- build dense connected layers in each Dense block
    for i = 1, nDenseConv do
       addDenseLayer(model, nDenseChannels, growthRate)
       nDenseChannels = nDenseChannels + growthRate
    end
    -- add transition layer
    model:add(nn.SpatialConvolution(nDenseChannels, growthRate, 1, 1, 1, 1, 0, 0))
    model:add(nn.ReLU(true))
    -- add constant scaling layer
    model:add(mulc(scaleRes, ipMulc))
