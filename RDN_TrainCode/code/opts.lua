...
function M.parse(arg)
	...
	cmd:option('-nTrain',           800,               'Number of images used for DIV2K training with data augmentation RF')
	...
	cmd:option('-valOffset',		800,				'(valOffset + 1) ~ (valOffset + nVal) images are used for validation,800')
    cmd:option('-nVal',             5,                 'Number of images for validation, 10')
    ...
    cmd:option('-localSkip',        'true',             'local skip connection in SDB')
    -- Loss
