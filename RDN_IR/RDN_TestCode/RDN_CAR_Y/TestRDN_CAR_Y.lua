...
cmd:option('-chopSize',     2e4,            'Minimum chop size for chopForward, 4e4')
...
local function saveImage(info, modelName)
    ...
    elseif opt.type == 'test' then
        local outputDir = paths.concat('img_output', modelName, info.setName, 'Q' .. opt.noise)
        if not paths.dirp(outputDir) then
            paths.mkdir(outputDir)
        end
...
elseif opt.type == 'test' then
    dataDir = paths.concat(opt.dataDir, opt.dataset, 'Q' .. opt.noise)
    --print(dataDir)
