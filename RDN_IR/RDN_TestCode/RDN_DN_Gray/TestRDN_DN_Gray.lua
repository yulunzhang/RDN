...
cmd:option('-dataDir',	    'LQ',      'data directory')
cmd:option('-dataset',      'CBSD68',       'test dataset: DIV2K | myData | Set5 | Set14 | B100 | Urban100')
...
cmd:option('-model',        'DNResNetR80P48M359',   'substring of model name')
cmd:option('-swap',         -1,             'Model swap')
cmd:option('-ensembleW',    '-1',           'Ensemble weight')
cmd:option('-scale',        1,              'scale factor: 2 | 3 | 4')
cmd:option('-noise',        50,             'noise level, 50')
cmd:option('-degrade',      'bicubic',      'degrading opertor: bicubic | unknown')

cmd:option('-chopShave',    10,             'Shave width for chopForward')
cmd:option('-chopSize',     1e4,            'Minimum chop size for chopForward, 4e4')
...
local util = require './functions/utils'(opt)
...
local function saveImage(info, modelName)
    ...
    elseif opt.type == 'test' then
        ...
        --local outputDir = paths.concat('img_output', modelName, info.setName, 'N' .. opt.noise)
        local outputDir = paths.concat('img_output', methodName, info.setName, 'N' .. opt.noise)
        if not paths.dirp(outputDir) then
            paths.mkdir(outputDir)
        end
        --local NameSave = string.gsub(info.fileName, 'LQ', modelName)
        local NameSave = string.gsub(info.fileName, 'LQ', methodName)
        image.save(paths.concat(outputDir, NameSave), info.saveImg:squeeze(1))
    end
    info.saveImg = nil
    collectgarbage()
    collectgarbage()
...
elseif opt.type == 'test' then
    dataDir = paths.concat(opt.dataDir, opt.dataset, 'N' .. opt.noise)
    --print(dataDir)
...
for i = 1, #testList do
    --print()
    testList[i].inputImg = image.load(paths.concat(testList[i].from, testList[i].fileName), 1, 'float'):mul(opt.mulImg)
end
