...
function util:calcPSNR(output, target, scale)
    ...
    elseif self.opt.dataset == 'div2k' or self.opt.dataset == 'flickr2k' or self.opt.dataset == 'SR291' or self.opt.dataset == 'cbsd432' then
...
function util:chopForward(input, model, scale, chopShave, chopSize, nGPU)
    ...
    local wc, hc = util:adaptive_shave(w, h)
...
function util:adaptive_shave(w, h)
    local shave_scale = 4
    local shave_size_max = 12
    local w_half, h_half = math.floor(w / 2), math.floor(h / 2)
    local mod_w, mod_h   = math.floor(w_half / shave_scale), math.floor(h_half / shave_scale)
    local wc = mod_w * shave_scale + shave_size_max
    local hc = mod_h * shave_scale + shave_size_max
    return wc, hc
end
