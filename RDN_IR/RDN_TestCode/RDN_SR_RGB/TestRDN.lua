...
cmd:option('-dataDir',	    'LR/LRBI',      'data directory')
cmd:option('-dataset',      'Set5',       'test dataset: DIV2K | myData | Set5 | Set14 | B100 | Urban100')
...
cmd:option('-model',        'RDN_BIX3',   'substring of model name')
cmd:option('-degradation',  'BI',   'degradation model')
...
local function saveImage(info, modelName)
    ...
    elseif opt.type == 'test' then
        ...
        local outputDir = paths.concat('SR', opt.degradation, methodName, info.setName, 'x' .. opt.scale)
        --local outputDir = paths.concat('SR', opt.degradation, modelName, info.setName, 'x' .. opt.scale)
        if not paths.dirp(outputDir) then
            paths.mkdir(outputDir)
        end
        local NameSave = string.gsub(info.fileName, 'LR' .. opt.degradation, methodName)
        --local NameSave = string.gsub(info.fileName, 'LR' .. opt.degradation, modelName)
...
elseif opt.type == 'test' then
    dataDir = paths.concat('LR/LR' .. opt.degradation, opt.dataset, 'x' .. opt.scale)
